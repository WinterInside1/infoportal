...
class UserService:
    @staticmethod
    async def create_user(user_create: CreateUserSchema) -> User:
    ...
    @staticmethod
    async def get_user_list(page: int, *, page_size: int) -> User:
        return await crud.user.get_multi(page=page, page_size=page_size)

    @staticmethod
    async def update_user(user_id: int, user_update: UpdateUserSchema) -> User:
        try:
            connection = psycopg2.connect(config.DB_USER,
                                          config.DB_PASS,
                                          config.DB_HOST,
